...
bool RenderSystem::init(GLFWwindow* window_arg)
{
    ...
    const int width = frame_buffer_width_px;
    const int height = frame_buffer_height_px;
	...
	// Set up texture parameters and allocate storage
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
		fprintf(stderr, "Framebuffer is %d\n", glCheckFramebufferStatus(GL_FRAMEBUFFER));
		assert(false);
	}
...
// 	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, frame_buffer_width_px, frame_buffer_height_px, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
// 	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
// 	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
// 	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, color_texture, 0);
//
// 	// Create a renderbuffer to use as a depth attachment
// 	GLuint depth_renderbuffer;
// 	glGenRenderbuffers(1, &depth_renderbuffer);
// 	glBindRenderbuffer(GL_RENDERBUFFER, depth_renderbuffer);
// 	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, frame_buffer_width_px, frame_buffer_height_px);
